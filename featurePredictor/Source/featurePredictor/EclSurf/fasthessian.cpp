...
FastHessian::FastHessian(cv::Mat *img, std::vector<Ipoint> &ipts,
	const int octaves, const int intervals, const int init_sample,
	const float thresh)
	: ipts(ipts), i_width(0), i_height(0)
{
...
void  FastHessian::cvmSet(cv::Mat* mat, int row, int col, double value)
{
	int type;
	...
	if (type == CV_32FC1)
		//주의
		((float*)(void*)(mat->data + (size_t)mat->step*row))[col] = (float)value;
	else
	{
		//주의
		assert(type == CV_64FC1);
		((double*)(void*)(mat->data + (size_t)mat->step*row))[col] = value;
	}
}